...
@frappe.whitelist()
def get_collection_entry(dt, dn, party_amount=None, bank_account=None, bank_amount=None):
	reference_doc = None
	doc = frappe.get_doc(dt, dn)
	if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) > 0:
		frappe.throw(_("Can only make payment against unbilled {0}").format(dt))
	...
	ce = frappe.new_doc("Payment Entry")
	ce.payment_type = payment_type
	ce.company = doc.company
	ce.cost_center = doc.get("cost_center")
	ce.posting_date = nowdate()
	ce.mode_of_payment = doc.get("mode_of_payment")
	ce.party_type = party_type
	ce.party = doc.get(scrub(party_type))
	ce.contact_person = doc.get("contact_person")
	ce.contact_email = doc.get("contact_email")
	ce.ensure_supplier_is_not_blocked()

	ce.paid_from = party_account if payment_type=="Receive" else bank.account
	ce.paid_to = party_account if payment_type=="Pay" else bank.account
	ce.paid_from_account_currency = party_account_currency \
		if payment_type=="Receive" else bank.account_currency
	ce.paid_to_account_currency = party_account_currency if payment_type=="Pay" else bank.account_currency
	ce.paid_amount = paid_amount
	ce.received_amount = received_amount
	ce.letter_head = doc.get("letter_head")

	if ce.party_type in ["Customer", "Supplier"]:
		bank_account = get_party_bank_account(pe.party_type, pe.party)
		ce.set("bank_account", bank_account)
		ce.set_bank_account_data()
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		...
		if (doc.doctype in ('Sales Invoice', 'Purchase Invoice')
			...
			for reference in get_reference_as_per_payment_terms(doc.payment_schedule, dt, dn, doc, grand_total, outstanding_amount):
				ce.append('references', reference)
		else:
			if dt == "Dunning":
				ce.append("references", {
					'reference_doctype': 'Sales Invoice',
					'reference_name': doc.get('sales_invoice'),
					"bill_no": doc.get("bill_no"),
					"due_date": doc.get("due_date"),
	...
	ce.setup_party_account_field()
	ce.set_missing_values()

	if party_account and bank:
		if dt == "Employee Advance":
			reference_doc = doc
		ce.set_exchange_rate(ref_doc=reference_doc)
		ce.set_amounts()
		if discount_amount:
			ce.set_gain_or_loss(account_details={
				'account': frappe.get_cached_value('Company', pe.company, "default_discount_account"),
				'cost_center': pe.cost_center or frappe.get_cached_value('Company', pe.company, "cost_center"),
				'amount': discount_amount * (-1 if payment_type == "Pay" else 1)
			})
			ce.set_difference_amount()

	if doc.doctype == 'Purchase Order' and doc.apply_tds:
		ce.apply_tax_withholding_amount = 1
		ce.tax_withholding_category = doc.tax_withholding_category

		if not ce.advance_tax_account:
			ce.advance_tax_account = frappe.db.get_value('Company', pe.company, 'unrealized_profit_loss_account')
