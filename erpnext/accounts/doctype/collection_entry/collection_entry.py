# -*- coding: utf-8 -*-
# Copyright (c) 2021, Code Disruptors, Inc. and contributors
...
class InvalidCollectionEntry(ValidationError):
	pass


class CollectionEntry(AccountsController):
	def __init__(self, *args, **kwargs):
		super(CollectionEntry, self).__init__(*args, **kwargs)
		if not self.is_new():
			...
			if d.allocated_amount:
				...
				for field, value in iteritems(ref_details):
					if d.exchange_gain_loss:
						...
						# and row.exchange_rate is already set to Collection Entry's exchange rate
	...
	def validate_paid_invoices(self):
		...
		for k, v in no_oustanding_refs.items():
			frappe.msgprint(
				_("{} - {} now have {} as they had no outstanding amount left before submitting the Collection Entry.")
					.format(k, frappe.bold(", ".join(d.reference_name for d in v)), frappe.bold("negative outstanding amount"))
				+ "<br><br>" + _("If this is undesirable please cancel the corresponding Collection Entry."),
...
def get_account_details(account, date, cost_center=None):
	frappe.has_permission('Collection Entry', throw=True)

	# to check if the passed account is accessible under reference doctype Collection Entry
	account_list = frappe.get_list('Account', {
		'name': account
	}, reference_doctype='Collection Entry', limit=1)

	# There might be some user permissions which will allow account under certain doctypes
	# except for Collection Entry, only in such case we should throw permission error
	if not account_list:
		frappe.throw(_('Account: {0} is not permitted under Collection Entry').format(account))
...
def get_payment_entry(dt, dn, party_amount=None, bank_account=None, bank_amount=None):
	...
	pe = frappe.new_doc("Collection Entry")
...
def make_payment_order(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		target.payment_order_type = "Collection Entry"
		target.append('references', dict(
			reference_doctype="Collection Entry",
	...
	doclist = get_mapped_doc("Collection Entry", source_name, {
		"Collection Entry": {
