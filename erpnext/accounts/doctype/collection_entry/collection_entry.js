...
frappe.ui.form.on('Collection Entry', {
	...
	setup: function(frm) {
		...
		frm.set_query('payment_term', 'references', function(frm, cdt, cdn) {
			...
			if (in_list(['Purchase Invoice', 'Sales Invoice'], child.reference_doctype) && child.reference_name) {
				let payment_term_list = frappe.get_list('Collection Schedule', {'parent': child.reference_name});
	...
	party: function(frm) {
		...
		if(frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			...
			return frappe.call({
				method: "erpnext.accounts.doctype.collection_entry.collection_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	set_account_currency_and_balance: function(frm, account, currency_field,
		...
		if (frm.doc.posting_date && account) {
			frappe.call({
				method: "erpnext.accounts.doctype.collection_entry.collection_entry.get_account_details",
				args: {
					"account": account,
					"date": frm.doc.posting_date,
					"cost_center": frm.doc.cost_center
	...
	get_outstanding_invoice: function(frm) {
		...
		const fields = [
			...
			{fieldtype:"Column Break"},
			{fieldtype:"Section Break"},
			{fieldtype:"Check", label: __("Allocate Collection Amount"), fieldname:"allocate_payment_amount", default:1},
		];
	...
	get_outstanding_documents: function(frm, filters) {
		...
		return  frappe.call({
			method: 'erpnext.accounts.doctype.collection_entry.collection_entry.get_outstanding_reference_documents',
			args: {
				args:args
			},
			callback: function(r, rt) {
	...
	set_deductions_entry: function(frm, account) {
		if(frm.doc.difference_amount) {
			frappe.call({
				method: "erpnext.accounts.doctype.collection_entry.collection_entry.get_company_defaults",
				args: {
					company: frm.doc.company
				},
				callback: function(r, rt) {
...
frappe.ui.form.on('Collection Entry Reference', {
	...
	reference_name: function(frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return frappe.call({
				method: "erpnext.accounts.doctype.collection_entry.collection_entry.get_reference_details",
				args: {
...
frappe.ui.form.on('Collection Entry Deduction', {
	amount: function(frm) {
		frm.events.set_unallocated_amount(frm);
	},
...
frappe.ui.form.on('Collection Entry', {
	cost_center: function(frm){
		if (frm.doc.posting_date && (frm.doc.paid_from||frm.doc.paid_to)) {
			return frappe.call({
				method: "erpnext.accounts.doctype.collection_entry.collection_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
